...
def ActiveInvestment(package):
    ...
    investment = db.investments.find_one({'$and' :[{'uid': uid},{'status' : 1},{'package':{'$gte': amount_package }} ]} )
    if investment is None:
...
def ActiveInvestmentSumit(package):
    ...
    #update balance 
            ...
            db.users.update({ "customer_id" : uid }, { '$set': { "balance_wallet": float(new_balance_wallet) ,"level" : int(package),"investment" : float(amount_package)} })
    #create investment
            data_investment = {
                'uid' : uid,
                'user_id': user['_id'],
