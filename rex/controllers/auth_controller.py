...
@auth_ctrl.route('/login-error', methods=['GET', 'POST'])
def loginerror():
    datass = {
      
    }
    
    return render_template('home/login_error.html', data=datass)
...
def signup(id_sponsor):
    ...
    if request.method == 'POST':
      ...
      if val_sponsor == '' and val_country == '' and val_email == '' and val_username == '' and val_password == ''  and val_recaptcha == '':
...
@auth_ctrl.route('/new/<positon>/<p_binary>/<sponsor>', methods=['GET', 'POST'])
def signup_intreessss(positon, p_binary, sponsor):
    ...
    if request.method == 'POST':
      ...
      if  val_country == '' and val_email == '' and val_username == '' and val_password == ''  and val_recaptcha == '':
        create_user_tree(sponsor,positon,p_binary,username,country,email,password)
        
        return redirect('/account/network-tree#register') 

    SITE_ROOT = os.path.realpath(os.path.dirname(__file__))
    json_url = os.path.join(SITE_ROOT, "../static", "country-list.json")
    data_country = json.load(open(json_url))

    sponser = db.users.find_one({'customer_id': sponsor})
    
    if sponser is None:
        return redirect('/account/dashboard')
    binary = db.users.find_one({'customer_id': p_binary})
    if binary is None:
        return redirect('/account/dashboard')
    else:
        if int(positon) == 1:
...
def create_user_tree(sponsor,position,p_binary,username,country,email,password):
  ...
  customer_find = db.users.find_one({'_id': ObjectId(customer)})
  
  if int(position)== 1:
      db.users.update({"customer_id": p_binary}, { "$set": { "left": customer_find['customer_id']} })
  else:
