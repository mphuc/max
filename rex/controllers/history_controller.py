...
@history_ctrl.route('/f1-earnings', methods=['GET', 'POST'])
def f1_earnings():
	if session.get(u'logged_in') is None:
		return redirect('/user/login')
	uid = session.get('uid')
	query = db.historys.find({'$and' : [{'type' : 'f1_earnings'},{'uid': uid}]})
	user = db.users.find_one({'customer_id': uid})
	list_notifications = db.notifications.find({'$and' : [{'read' : 0},{'status' : 0},{'$or' : [{'uid' : uid},{'type' : 'all'}]}]})
	number_notifications = list_notifications.count()
	data ={
	...
	return render_template('account/f1_earnings.html', data=data)
